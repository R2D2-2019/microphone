...
namespace r2d2::microphone {
  ...
  private:
    hwlib::adc &mic;
    static constexpr int buffer_size = 63; // size of internal communication
                                           // buffer, this size must not be
                                           // changed!!
    std::array<uint16_t, buffer_size>
        sample_buffer;  // array storing our samples
  ...
  public:
     ...
     */
    microphone_controller_c(hwlib::adc &new_mic) : mic(new_mic) {}
     ...
     */
    constexpr int get_buffer_size() const { return buffer_size; }
